...
class CountryCard extends React.Component{
    render(){
        ...
        return(
             <div className=" country-card rounded relative  shadow  ">
   ...
   componentDidMount() {
    ...
    searchCountries = (event) =>{
        ...
        this.countriesToRender = this.state.countryList.filter((country)=>{
            return(country.name.includes(keyword));
        });
        }
    }
    changeTheme = () =>{
        ...
        return(
            <div className={this.state.theme==='light'?"bg-light text-white":"bg-dark text-dark"}>
                <header className="d-flex justify-content-between position-relative px-lg-10 px-xl-16 py-8 px-5 shadow p-4 mb-5  rounded align-items-center ">
                    <a aria-current="page" className={this.state.theme==='light'?"heading px-3  text-xl-left text-dark":"heading px-3  text-xl-left text-light"} href="/">Where in the world?</a>
                <div className="d-flex text-capitalize px-3 align-items-center">
                    ...
                    <p onClick={this.changeTheme} className={currentTheme==='light'?"text-dark bg-light text-capitalize text-center m-1 toggle":"toggle text-light bg-dark text-capitalize text-center m-1 "}> light mode</p>
                </div>
                </header>
                <main className=" px-5 text-light px-lg-10 px-xl-16  w-100">
                    <div className="d-flex px-3 justify-content-between align-items-center mb-3">
                        <div  className="d-flex px-3 flex-wrap rounded shadow bg-white w-20 md:w-auto align-items-center">
                        <img className="search w-10" src="https://res.cloudinary.com/dqcsk8rsc/image/upload/v1578427182/Search_1_1_1_dms40o.svg"></img>{/*eslint-disable-next-line*/}
                            <input onKeyDown={this.searchCountries} className="text-grey w-90 light-mode-elements bg-left bg-no-repeat rounded  py-3 px-10 border-0 light-mode-elements mb-10 md:mb-0 " type="text" id="country" placeholder="Search for a country..." ></input>
                        </div>
                        <select onChange={this.changeRegion} name="region" id="" className="m-2 bg-white border-0 px-4 py-2 light-mode-elements shadow p-3  bg-light rounded w-8/12 md:w-auto">
                    ...
                    <div className={this.state.theme==='light'?"day-countries text-dark w-100 countries p-3 md:px-0":"night-countries text-light w-100 countries p-3 md:px-0"}>
                        {this.countriesToRender.map((country)=>{
                            return <CountryCard key={country.numericCode} name={country.name} flag={country.flag} region={country.region} population={country.population} capital={country.capital}/>
                        })}
                    </div>
                </main>
            </div>
